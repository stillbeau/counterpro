...
def calculate_cost(unit_cost_from_csv, project_sqft):
    ...
    # IB Formula: ((UnitCost * IB_MATERIAL_MARKUP) + FABRICATION_COST_PER_SQFT) * project_sqft
...
def parse_product_variant(variant_str):
    ...
    # 3. Clean up Name
    # Remove "15 - ", remove thickness, remove location code
    ...
    # Simple split for Brand/Color (Assumes Brand is first word)
...
def load_and_clean_data(uploaded_file):
    ...
    try:
        ...
        # Try skipping rows if header isn't on row 1
    ...
    if 'Product Variant' not in df.columns:
        return None, "Error: Could not find 'Product Variant' column."
    ...
    for col in ['On Hand Qty', 'Serialized On Hand Cost']:
        ...
        if found_col:
            df[found_col[0]] = pd.to_numeric(df[found_col[0]].astype(str).str.replace(r'[$,]', '', regex=True), errors='coerce')
            df.rename(columns={found_col[0]: col}, inplace=True)
...
st.markdown("Upload your monthly inventory CSV to generate quotes.")
